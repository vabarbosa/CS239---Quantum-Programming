...
def compile_simon(Uf_quil_def, n, time_out_val = 100):
    """
...
def simon(num_bits, qc, executable):
    ...
    if not isinstance(qc, QuantumComputer):
        raise ValueError("qc must be a Quantum Computer")
    if not isinstance(executable, rpcq.messages.PyQuilExecutableResponse):
        raise ValueError("executable must be of type PyQuilExecutableResponse")
    ...
    if not solver.is_ready():
        pr>int("Failure: Measured %d times and only got %d independent y's." % (max_measurements, len(solver.independent_equations)))
    s = solver.solve()
    time_elapsed = int((time.time() - start) * 1000)
...
def plot_uf_variance():
    """
    Plot histogram of Uf running times
    Returns: 
        None
    """
    ...
    plt.hist(time_array)
    plt.title('Dependence of execution time on $U_f$ (Simon algorithm)', fontsize=25)
...
def plot_time(npz_path):
    """
    Plot running times of test data
    Args:
        npz_path: path to npz_file to load with data 
    Returns: 
        None
    """
...
def true_s(f, s_calc):
    """
    Check if f(0) == f(s). If it's not s = 0^n.
    Args:
        f: 2^n x n array representing function
        s_calc: predicted s to confirm
    Returns: 
        true value of s - either 0^n or s_calc
    """
...
def run_tests(npz_file):
    """
    Test driver for circuit. For n = 1 through 3, for each 2^n possible s, choose an f at random, compile 5 times and run 5 times
    Args:
        npz_path: path to npz_file to save with data 
    Returns: 
        None
    """
    time_out_val = 10000
...
if __name__ == "__main__":
    TEST_DRIVER = True
    PLOT = False
    npz_file = 'simon_benchmarking.npz'

    if TEST_DRIVER:
        run_tests(npz_file)
    if TEST_DRIVER and PLOT:
        plot_time(npz_file)
        plot_uf_variance()
